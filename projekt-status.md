...
**Næste (blokerende for launch):** oversæt `src/components/templates/` (4 artikelskabeloner), `src/components/profiles/` (atlet+skole), `/viden` og `[...segments]`-metadata + skriv de statiske sider på engelsk med `country='UK'`. Ufarligt lige nu, fordi der ikke findes britisk indhold at vise. Fremgangsmåden er mekanisk (nøgle i `UiKey` → oversæt i begge pakker → `t(...)`).
