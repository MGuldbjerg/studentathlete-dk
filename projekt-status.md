...
**Næste (blokerende for launch) — nu INDHOLD, ikke kode:** `/viden`-guiderne på engelsk · de statiske sider (om/kontakt/ai-brug/presseetik/cookies) seedet med `country='UK'` · `[...segments]`-metadata (danske meta-beskrivelser). Derefter: DNS/zone/wrangler-route (trin 5 i `SETUP-uk-launch.md`) og engelske kladder til gennemlæsning.
