...
- **ADSENSE-SCRIPTET ER TÆNDT (2026-08-04, Worker 275114dc)**: `<script async src=".../adsbygoogle.js?client=ca-pub-6062052231600117" crossorigin>` i `<head>`, verificeret live. Styret af **admin → Tekster → «Indlæs AdSense-scriptet»** (`adsense.enabled`) — kan slukkes igen UDEN deploy. Tænder auto ads + Googles samtykkeboks i ét.
  ...
  - **⚠️ ÅBENT: ingen "skift dit samtykke"-knap.** Teksten siger derfor ærligt at man rydder browserens cookies for at blive spurgt igen. GDPR vil have at tilbagetrækning er lige så let som samtykke → tilføj et link der genåbner Googles boks (Funding Choices' `googlefc`-API) når du får lyst. Indtil da er teksten korrekt, men løsningen er klodset for læseren.
